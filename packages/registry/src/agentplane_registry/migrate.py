"""Programmatic Alembic migrations (SPEC §5.2): the schema is migration-managed.
...
runtime's ``migrate.py`` — services never import each other (invariant 3).
"""
...
_MARKER_TABLE = "entries"  # present in every pre-Alembic registry database
